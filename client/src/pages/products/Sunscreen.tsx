...
const Sunscreen = () => {
  const ingredientData = {
    "Zinc Oxide": {
      title: "Zinc Oxide",
      description: "Mineral UV filter that provides broad-spectrum protection.",
      benefits: ["UVA/UVB Protection", "Soothing"]
    },
    "Vitamin E": {
      title: "Vitamin E",
      description: "Antioxidant that helps protect from environmental stressors.",
      benefits: ["Antioxidant", "Skin Conditioning"]
    },
    "All Ingredients": {
      title: "Complete Ingredient List",
      description: "List your sunscreen ingredients here...",
      benefits: []
    }
  } as const;

  const features = [
    { text: "Broad Spectrum Protection", icon: ShieldCheck },
    { text: "Lightweight Texture", icon: Feather },
    { text: "No White Cast", icon: Palette },
    { text: "Non-Greasy Finish", icon: Scale },
    { text: "Everyday Wear", icon: Sparkles },
  ];

  const [selectedIngredient, setSelectedIngredient] = useState<keyof typeof ingredientData>("Zinc Oxide");
  ...
  return (
    <>
      <ProductPage slug="sunscreen" />

      {/* Why You'll Love It */}
      <section className="max-w-7xl mx-auto mt-12 px-4">
        <div className="grid md:grid-cols-2 gap-6 items-start">
          <div className="relative group rounded-2xl overflow-hidden">
            <img src={whyLoveImg} alt="Sunscreen Visual" className="w-full h-full object-cover rounded-2xl" />
          </div>
